...
export interface MutationConfiguration<TData = unknown, TVariables = unknown, TContext = unknown> {
...
export function createMutationOptions<TData = unknown, TVariables = unknown, TContext = unknown>(
...
export const retryStrategies: Record<string, RetryConfiguration> = {
  network: {
    ...
    condition: (error: Error) => error.name === 'NetworkError' || error.message.includes('network'),
  },
  server: {
    attempts: 2,
    delay: (attempt: number) => 1000 * attempt,
    condition: (error: Error) => error.message.includes('5') && error.message.includes('server'),
  },
  validation: {
    attempts: 0,
    delay: () => 0,
...
export function shouldRetry(error: Error, attempt: number, strategy: keyof typeof retryStrategies): boolean {
  const config = retryStrategies[strategy]
  return attempt < config.attempts && config.condition(error)
}

export function getRetryDelay(attempt: number, strategy: keyof typeof retryStrategies): number {
  const config = retryStrategies[strategy]
